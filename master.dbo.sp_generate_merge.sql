CREATE PROCEDURE dbo.[GENERATE_TABLE_DATA_MERGE]
(
...
/***********************************************************************************************************
Procedure: [GENERATE_TABLE_DATA_MERGE]
 (Adapted by Isamu Ybarra for SQL Server 2005)
 * Converted IIF
 * Fixed order of VALUES to be consistent, otherwise ended up with , before first VALUE statement
 * Removed MASTER db logic
...
DECLARE @ignore_disable_constraints BIT = 
	CASE WHEN OBJECT_ID(@Source_Table_Qualified, 'U') IS NULL
	THEN 1
	ELSE 0
	END
...
BEGIN
	...
	ELSE
	BEGIN
		DECLARE @All_Constraints_Enabled BIT = 
			CASE WHEN (SELECT COUNT(1) FROM @Source_Table_Constraints WHERE [is_disabled] = 0) = @Constraint_Ct
			THEN 1
			ELSE 0
		END

		DECLARE @All_Constraints_Trusted BIT = 
			CASE WHEN (SELECT COUNT(1) FROM @Source_Table_Constraints WHERE [is_not_trusted] = 0) = @Constraint_Ct
			THEN 1
			ELSE 0
		END
		DECLARE @All_Constraints_NotTrusted BIT = 
			CASE WHEN (SELECT COUNT(1) FROM @Source_Table_Constraints WHERE [is_not_trusted] = 1) = @Constraint_Ct
			THEN 1
			ELSE 0
		END
